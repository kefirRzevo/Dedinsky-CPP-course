...
bool LineCrossSphere(Vector l1, Vector l2, double radius, Vector r0,
                     Vector &point) {
...
Vector CountIntensity(Vector pixel, int n_spheres, Sphere *spheres, Lamp lamp,
                      Camera camera) {
  ...
  for (int i = 0; i < n_spheres; i++) {
    if (!LineCrossSphere(camera_point, pixel, spheres[i].radius,
                         spheres[i].center, cross_point))
      continue;
  ...
  Vector reflected_beam =
      2 * ScalarMul(normal, incident_beam) * normal - incident_beam;
  Vector camera_beam = lamp.position - cross_point;

  double D = Cos(normal, incident_beam);
  double S = std::pow(Cos(reflected_beam, camera_beam), MAX_REFLECTIONS_COUNT);

  return (D * spheres[nearest_sphere].Kd + lamp.Ka) *
             ComponentMul(spheres[nearest_sphere].color, lamp.color) +
         S * spheres[nearest_sphere].Ks * lamp.color;
}
...
Pixel VectorToPix(Vector v, double Alpha) {
  ...
  return Pixel((sf::Uint8)(kR * 255), (sf::Uint8)(kG * 255),
               (sf::Uint8)(kB * 255), (sf::Uint8)(Alpha * 255));
}
