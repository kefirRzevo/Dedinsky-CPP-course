...
struct Node {
  ...
  Node(NodeType type_, string name_ = "") : name(name_), type(type_) {};
};
...
struct Call {
  ...
  Call(string ident_) : ident(ident_) {}
};
...
public:
  ...
  static Dumper *GetDumper() {
    ...
    if (!dumper)
      dumper.reset(new Dumper("build/test.dot"));

    return dumper.get();
  }

  Node &GetNode() { return nodes.back(); }

  void StartCall(const string &ident) {
    callVector.push_back(AddCall(ident));
  }

  void EndCall() { callVector.pop_back(); }
  ...
  void Dump() {
    ...
    MakeIndent(0);
    fout
        << R"(node [penwidth = 2, shape = box, fillcolor = white, style = "rounded, filled", fontname = "Courier"])";
    MakeIndent(0);
    DumpCallData(call);
    MakeIndent(0);
...
struct Scope {
public:
  Scope(const string &ident) { Dumper::GetDumper() -> StartCall(ident); }

  ~Scope() { Dumper::GetDumper() -> EndCall(); }
};
